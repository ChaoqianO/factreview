...
:class:`Claim` objects. Depending on :class:`FactExtractionCfg.mode`,
the extractor runs:
...
from factreview.schemas.paper import Paper, ReportedResult
...
def extract_facts(
    paper: Paper,
    *,
    cfg: FactExtractionCfg | None = None,
    ...
    ----------
    paper
        The structured paper produced by :mod:`factreview.ingestion`.
    cfg
        Fact-extraction sub-config. Defaults to :class:`FactExtractionCfg`
        with ``mode="auto"`` and decomposition enabled.
    llm_cfg
        LLM routing sub-config; only consulted when mode ∈ {auto, llm}.
    """
    cfg = cfg or FactExtractionCfg()
