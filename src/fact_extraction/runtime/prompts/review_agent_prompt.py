...
def _build_fact_review_extractor_prompt(
    *,
    ...
    return (
        ...
        "- `2. Technical Positioning`: never place the current paper title as an external row. Use only `This Work` for self-row identity.\n"
        ...
        "- `5. Experiment`: extract and display all reported experimental-result tables/rows from the manuscript main text; do not sample, summarize-only, or truncate.\n"
        ...
        "- Under `Ablation Result`, then provide markdown tables covering every ablation/sensitivity/variant result using fixed columns exactly `Ablation Dimension | Configuration | Full Model | Paper Result | Difference (Δ)`.\n"
