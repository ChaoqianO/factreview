...
def _build_fact_review_extractor_prompt(
    *,
    ...
    return (
        ...
        "- Use the provided Semantic Scholar retrieval block for objective related-work comparison in section 2.\n"
        ...
        "  Step 2: use `[Semantic Scholar Retrieval]` papers as objective top related works.\n"
        ...
        "- `2. Technical Positioning`: row semantics are strict: `Research domain` = field/subfield category; `Method` = concrete model/paper name.\n"
        ...
        "- `2. Technical Positioning`: niche-dimension column names must be plain content labels only (no IDs such as R1/R2/R3, no legend).\n"
        ...
        "- `3. Claims`: provide a markdown table with columns `Claim | Evidence | Location` only. Do not include `Assessment` or `Status` columns.\n"
        "- `4. Summary`: first give a short summary paragraph, then include two sublabels exactly named `Strengths:` and `Weaknesses:` followed by bullet lists.\n"
        "- `5. Experiment`: include two subsection labels exactly named `Main Result` and `Ablation Result`.\n"
        "- `5. Experiment`: include all reported main and ablation experiment rows from the manuscript tables; do not sample or truncate.\n"
        "- Under `Main Result`, include a `Location:` line first, then provide a markdown table with columns `Task | Dataset | Metric | Best Baseline | Paper Result | Difference (Δ)`.\n"
        ...
        "- Under `Ablation Result`, then provide a markdown table with columns `Ablation Dimension | Configuration | Full Model | Paper Result | Difference (Δ)`.\n"
