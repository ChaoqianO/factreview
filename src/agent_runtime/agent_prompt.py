...
def _build_fact_review_extractor_prompt(
    *,
    ...
    return (
        ...
        "- `2. Technical Positioning`: after the first two columns, dynamically append 3-8 niche-dimension columns derived from manuscript claims + objective retrieval comparison.\n"
        ...
        "- `2. Technical Positioning`: cells in niche-dimension columns must use only `√` or `×`.\n"
        ...
        "- `5. Experiment`: first classify each manuscript result table/row as `main` or `ablation` before writing any table.\n"
        "- `5. Experiment`: extract and display all reported experimental-result tables/rows from the manuscript main text after classification; do not sample, summarize-only, or truncate valid main-result rows.\n"
        "- Under `Main Result`, include a `Location:` line first, then provide markdown tables covering every non-ablation experimental result using fixed columns exactly `Task | Dataset | Metric | Best Baseline | Paper Result | Difference (Δ)`.\n"
        "- `Main Result` inclusion rule: include benchmark/baseline/comparison/result tables that report task-dataset-metric outcomes; exclude controlled removal, sensitivity, hyperparameter, architecture-variant, data-subset, retraining, or one-factor-variant tables and put those only under `Ablation Result` if they satisfy the ablation rules.\n"
        "- Under `Ablation Result`, include a `Location:` line first. This line must indicate where the ablation is reported and on which dataset(s) it is evaluated in the manuscript.\n"
        "- Under `Ablation Result`, then provide markdown tables using fixed columns exactly `Ablation Dimension | Configuration | Full Model | Paper Result | Difference (Δ)`.\n"
        "- `Ablation Result` selection protocol: only include controlled ablation/sensitivity/variant groups where rows share the same task, dataset, and metric, and each reported variant changes exactly one factor relative to a full-model anchor.\n"
        "- `Ablation Result` full-model anchor: use the manuscript's explicit full/ours/best complete configuration when available; otherwise choose the best-performing complete configuration in that ablation group as `Full Model`.\n"
        "- `Ablation Result` row rule: each row must compare one modified configuration against the selected `Full Model`; do not include broad benchmark rows, per-class result rows, qualitative visualization-only rows, or unrelated table summaries as ablations.\n"
        "- `Ablation Result`: if no valid controlled ablation group exists, output one placeholder row filled with `Not found in manuscript` rather than converting main results into ablations.\n"
