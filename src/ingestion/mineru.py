...
from util.runner import CommandResult, persist_command_result, run_command
...
def extract_with_mineru(
    *,
    ...
    backend = str(os.getenv("CODE_EVAL_MINERU_BACKEND") or "pipeline").strip()
    device = str(os.getenv("CODE_EVAL_MINERU_DEVICE") or "cpu").strip()
    source = str(os.getenv("CODE_EVAL_MINERU_SOURCE") or "").strip()  # huggingface/modelscope/local
        ...
        for i, b in enumerate(blocks, 1):
            ...
            try:
                ...
                out_html_path.write_text(
                    "<!-- generated by code_evaluation MinerU postprocess -->\n"
                    + (f"<!-- {caption} -->\n" if caption else "")
                    + table_html
                    + "\n",
                    encoding="utf-8",
