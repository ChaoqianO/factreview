...
from fact_extraction.adapters.markdown_parser import build_page_index
from fact_extraction.adapters.mineru import MineruAdapter, MineruConfig
from fact_extraction.adapters.paper_search import (
    PaperReadConfig,
    PaperSearchAdapter,
    PaperSearchConfig,
)
from fact_extraction.adapters.semantic_scholar import SemanticScholarAdapter, SemanticScholarConfig
from fact_extraction.config import get_settings
from fact_extraction.prompts.review_agent_prompt import build_review_agent_system_prompt
from fact_extraction.report.review_report_pdf import build_review_report_pdf
from fact_extraction.report.source_annotations import build_source_annotations_for_export
from fact_extraction.state import ensure_artifact_paths, fail_job, load_job_state, mutate_job_state, set_status
from fact_extraction.storage import append_event, read_json, write_json_atomic, write_text_atomic
from fact_extraction.tools.review_tools import ReviewRuntimeContext, build_review_tools
from fact_extraction.types import AnnotationItem, JobStatus
...
def _latest_code_eval_payload(*, project_root: Path, source_pdf_name: str) -> tuple[dict[str, Any], dict[str, Any]]:
    paper_key = Path(str(source_pdf_name or 'paper')).stem.strip().lower() or 'paper'
