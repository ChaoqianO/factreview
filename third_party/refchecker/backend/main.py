...
def _load_dotenv_files() -> None:
    ...
    Why: On Windows/dev setups users often expect `.env` to be picked up automatically.
    The Web UI relies on backend env vars for seeding a default LLM config.

    Rules:
    - Only sets variables that are not already set in the environment
    - Supports simple KEY=VALUE lines (quotes optional), ignores comments/empty lines
    """
    candidates = []
    cwd = Path.cwd()
    candidates.extend([cwd / ".env", cwd / ".env.local"])

    # Repo root (when running from a cloned repo)
    try:
    ...
    for path in candidates:
        ...
        try:
            ...
            for raw in path.read_text(encoding="utf-8").splitlines():
                ...
                # Strip BOM if file was saved as UTF-8 with BOM on Windows
                key = key.lstrip("\ufeff").strip()
                value = value.strip()
                # If value is unquoted, support trailing inline comments: KEY=value  # comment
...
async def _seed_default_llm_config_from_env() -> None:
    """If the DB has no LLM configs, create a default one from env vars.

    This makes the Web UI "just work" for devs who set keys in `.env` / env vars,
    without requiring manual creation via the Settings panel.
    """
    try:
        existing = await db.get_llm_configs()
        if existing:
            return

        # Support a common legacy env var name used by some setups.
        if not os.getenv("REFCHECKER_LLM_PROVIDER") and os.getenv("MODEL_PROVIDER"):
            os.environ["REFCHECKER_LLM_PROVIDER"] = os.getenv("MODEL_PROVIDER", "")

        provider = (os.getenv("REFCHECKER_LLM_PROVIDER") or "").strip().lower()
        model = (os.getenv("REFCHECKER_LLM_MODEL") or "").strip() or None

        # Infer provider if not specified
        if not provider:
            if os.getenv("OPENAI_API_KEY") or os.getenv("REFCHECKER_OPENAI_API_KEY"):
                provider = "openai"
            elif os.getenv("ANTHROPIC_API_KEY") or os.getenv("REFCHECKER_ANTHROPIC_API_KEY"):
                provider = "anthropic"
            elif os.getenv("GOOGLE_API_KEY") or os.getenv("REFCHECKER_GOOGLE_API_KEY"):
                provider = "google"
            elif os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("REFCHECKER_AZURE_API_KEY"):
                provider = "azure"
            elif os.getenv("REFCHECKER_VLLM_SERVER_URL"):
                provider = "vllm"
        ...
        endpoint = None
        if provider == "openai":
            api_key = os.getenv("OPENAI_API_KEY") or os.getenv("REFCHECKER_OPENAI_API_KEY")
            endpoint = os.getenv("OPENAI_BASE_URL") or os.getenv("REFCHECKER_OPENAI_BASE_URL")
            if not model:
                model = os.getenv("OPENAI_MODEL") or None
        elif provider == "anthropic":
            api_key = os.getenv("ANTHROPIC_API_KEY") or os.getenv("REFCHECKER_ANTHROPIC_API_KEY")
        elif provider == "google":
            api_key = os.getenv("GOOGLE_API_KEY") or os.getenv("REFCHECKER_GOOGLE_API_KEY")
        elif provider == "azure":
            api_key = os.getenv("AZURE_OPENAI_API_KEY") or os.getenv("REFCHECKER_AZURE_API_KEY")
            endpoint = os.getenv("AZURE_OPENAI_ENDPOINT") or os.getenv("REFCHECKER_AZURE_ENDPOINT")
        elif provider == "vllm":
            # vLLM provider doesn't require an API key; endpoint is server URL.
            endpoint = os.getenv("REFCHECKER_VLLM_SERVER_URL") or "http://localhost:8000"

        if provider in ("openai", "anthropic", "google", "azure") and not api_key:
            logger.info(f"Not seeding LLM config for provider={provider}: missing API key env var")
            return

        name = f"From env ({provider})"
        ...
        await db.set_default_llm_config(config_id)
        logger.info(f"Seeded default LLM config id={config_id} provider={provider} model={model or 'default'}")
    except Exception as e:
        logger.warning(f"Failed to seed default LLM config from env: {e}")
...
async def startup_event():
    ...
    # Load .env before doing anything else (so env-based settings are available)
    _load_dotenv_files()
    await db.init_db()
    ...
    # Seed a default LLM config from environment variables (dev convenience)
    await _seed_default_llm_config_from_env()
