...
class OpenAIProvider(LLMProvider, LLMProviderMixin):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Support OpenAI-compatible endpoints (proxies, self-hosted gateways, etc.)
        # - Web UI passes this as `endpoint`
        # - CLI/env supports `OPENAI_BASE_URL` or `REFCHECKER_OPENAI_BASE_URL`
        self.base_url = (
            config.get("base_url")
            or config.get("endpoint")
            or os.getenv("REFCHECKER_OPENAI_BASE_URL")
        ...
        if self.api_key:
            try:
                import openai
                # The OpenAI SDK supports `base_url` for OpenAI-compatible servers.
                # If base_url is None, it defaults to OpenAI's official endpoint.
