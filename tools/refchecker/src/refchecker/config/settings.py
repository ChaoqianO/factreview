...
def get_config() -> Dict[str, Any]:
    ...
    # Optional: OpenAI-compatible base URL (for proxies/self-hosted gateways)
    # This is consumed by the OpenAI provider as `base_url`.
    if os.getenv("OPENAI_BASE_URL") or os.getenv("REFCHECKER_OPENAI_BASE_URL"):
        config["llm"]["openai"]["base_url"] = os.getenv("OPENAI_BASE_URL") or os.getenv("REFCHECKER_OPENAI_BASE_URL")

    # Optional: convenience env var for OpenAI model. Prefer the unified REFCHECKER_LLM_MODEL.
    if os.getenv("OPENAI_MODEL") and not os.getenv("REFCHECKER_LLM_MODEL"):
        config["llm"]["openai"]["model"] = os.getenv("OPENAI_MODEL")
